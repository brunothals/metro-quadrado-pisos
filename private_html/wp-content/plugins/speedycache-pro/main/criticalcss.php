...
class CriticalCss extends CommonCss{
	...
	static function generate($urls){
		...
		if(empty($speedycache->license['license']) || empty($speedycache->license['active'])){
			self::log('speedycache_ccss_logs', 'License Not found or inactive, please link your License');
			return false;
		}
		...
		$license = $speedycache->license['license'];
		...
		foreach($urls as $url){
			...
			$last_request = get_transient('speedycache_critical_css_time');

			// Make the request wait for atleast 10 seconds
			if(!empty($last_request) &&  (time() - $last_request) < 10){
				return;
			}

			set_transient('speedycache_critical_css_time', time(), 90);
	...
	<div modal-id="speedycache_critical_css" class="speedycache-modal">
		<div class="speedycache-modal-wrap">
			...
			if(!is_array($generate_ccss) || count($generate_ccss) < 1){
