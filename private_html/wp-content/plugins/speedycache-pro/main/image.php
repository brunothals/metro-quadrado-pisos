...
class Image{
	...
	static function start_optimization($img){
		...
		if(empty($speedycache->image['settings']['compression_method'])){
			return array('success' => false,  'error_message' => __('The provided conversion method is not valid', 'speedycache-pro'));
		}
	...
	static function revert(){
		...
		foreach($optm_json as $key => $image){
			...
			if(
				...
				(pathinfo($image['file'], PATHINFO_EXTENSION) == 'webp')
			){
				@unlink($image['file']);
	...
	static function allowed_mime($filename){
		...
		if(!empty($speedycache->image['settings']['compression_method']) && $speedycache->image['settings']['compression_method'] === 'cwebp' && self::gif2webp_exists()){
	...
	static function get_child_row(){
		...
		return '<tr class="alternate author-self status-inherit" post-id="{{post_id}}" post-type="detail" style="display: none; padding-left: 20px;">
			<td class="column-icon media-icon" style="font-size:3.5em; color:#ccc;"><span class="dashicons dashicons-format-image"></span></td>
			<td class="title column-title">
				<p style="margin-bottom: 0;"><a href="{{url}}" target="_blank">{{post_title}}</a></p>
				<p style="margin-bottom: 0;">
					<a style="{{backup_error_style}}" href="{{backup_url}}" target="_blank"><strong>{{backup_title}}</strong></a>
		...
		<div class="speedycache-block">
			...
			<form class="speedycache-img-opt-settings">
				...
				<div class="speedycache-block-title" style="flex-direction:column">
					...
					<p style="display:block; font-family: monospace; background-color: #f2f2f2; border-radius:6px; padding:10px;">
						<span style="color:var(--speedycache-red); font-weight:bold;"><span class="dashicons dashicons-info-outline"></span> Note: </span>
						<?php 
							esc_html_e('Only cwebp supports gif conversions to webp. But when you upload an image to ' . (defined('SITEPAD') ? 'Sitepad' : 'WordPress') . ' whether it be a jpg, png, or gif format, ' . (defined('SITEPAD') ? 'Sitepad' : 'WordPress') . ' creates different sizes of that image on upload so, in the process of resizing some resized version of gifs may lose their animation.', 'speedycache-pro');
						?>
					</p>
				</div>
					...
					<table style="margin:auto; width: 100%;">
						...
						<tbody>
							...
							<tr>
								...
								<td class="speedycache-table-item" style="text-align:right;"><?php echo esc_html($time_str);?> <span class="dashicons dashicons-clock"></span></td>
							</tr>
							<?php } ?>
			...
			<div class="speedycache-modal-wrap">
				<div class="speedycache-modal-content">
					...
					<div class="speedycache-modal-db-actions">
						<button class="speedycache-button speedycache-btn-black speedycache-db-confirm-yes"><?php esc_html_e('Yes', 'speedycache'); ?></button>
						<button class="speedycache-button speedycache-db-confirm-no"><?php esc_html_e('No', 'speedycache'); ?></button>
					</div>
				</div>
			</div>
		</div>
	...
	static function convert_on_upload($id){
		...
		if(empty($speedycache->image['settings']['on_upload'])){
			return;
		}
	...
	static function optimize_on_fly($content){
		...
		$queued_images = 0;
		foreach($matches as $match){
			// If the page has a lot of images, then the process could get slower.
			if($queued_images >= 10){
				break;
			}
			...
			$optimized = (int) $wpdb->get_var($wpdb->prepare("SELECT 1 as optimized FROM `".$wpdb->prefix."postmeta` WHERE `meta_key`= 'speedycache_optimisation' AND `post_id`=%d LIMIT 1", $post_id));
